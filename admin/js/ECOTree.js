		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					this.ctx.canvas
					this.ctx.fillText(node.dsc,(node.XPosition+this.canvasoffsetLeft+5),(node.YPosition+this.canvasoffsetTop+10));
					s.push('</font>');
					s.push('</div>');		
					break;
				...
				case "VML":
					...
					if (node.canCollapse) {
						...
						s.push('</a>');
						s.push('<img style="width:auto" src="'+this.config.transImage+'" >');	
					}					
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<div style="padding-left:-500px;position:absolute;">');
			s.push('<center><canvas id="ECOTreecanvas" width=5000 height=500 ></canvas></center>');	
			s.push('</div>');
			break;
		...
		case "VML":
			s.push('<v:group coordsize="10000, 10000" coordorigin="0, 0" style="position:relative;width=10000px;height=10000px;" >');
			s.push(this._drawTree());
			s.push('</v:group>');
			break;
	}
