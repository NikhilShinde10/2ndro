...
class MultiModelForm extends CWidget
{
    ...
    public $jsAfterNewId;
    public $jsAfterNewId_AutoComplete;
    public $jsAfterNewId_AutoCompleteForNew;
    ...
    public static function afterNewIdDateTimePicker($element)
    {
     ...
     public static function afterNewIdTimePicker($element)
    {
        $options = isset($element['options']) ? $element['options'] : array();
        $jsOptions = CJavaScript::encode($options);
        ...
        $language = isset($element['language']) ? $element['language'] : '';
        if (!empty($language))
            $language = "jQuery.timepicker.regional['$language'],";

        return "if(this.hasClass('hasDatepicker')) {this.removeClass('hasDatepicker').timepicker($jsOptions);};";

       // return "if(this.hasClass('hasTimepicker')) {this.removeClass('hasTimepicker').timepicker(jQuery.extend($language {$jsOptions}));};";
    }	
   ...
   public static function afterNewIdAutoComplete($element)
    ...
    public static function afterNewIdAutoCompleteForNew($element)
    {
        $options = isset($element['options']) ? $element['options'] : array();
        if (isset($element['sourceUrl']))
            $options['source'] = CHtml::normalizeUrl($element['sourceUrl']);
...
//        $link = CHtml::link($this->removeText, '#', $htmlOptions);
        $link = CHtml::image('images/delete.png','', $htmlOptions);
    ...
    protected function getClientOptions()
    {
        ...
        if (!empty($this->jsAfterNewId_AutoComplete))
	{
		if(empty($this->options['afterNewId']))
		{
			$this->options['afterNewId']="";
		}		
            $this->options['afterNewId'].= $this->jsAfterNewId_AutoComplete;
	}

        if (!empty($this->jsAfterNewId_AutoCompleteForNew))
	{
		if(empty($this->options['afterNewId']))
		{
			$this->options['afterNewId']="";
		}		
            $this->options['afterNewId'].= $this->jsAfterNewId_AutoCompleteForNew;
	}
	...
	if($this->extra_button==true)
		$cs->registerScript(__CLASS__ . '#' . $this->id."1", "jQuery('#{$this->id}1').relCopy($options);");
