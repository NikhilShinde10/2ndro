...
class Blog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, description,created_at, active, logo,date,category,author', 'required'),
			array('active,sort', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>500),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title,  description, date, created_at, active,category,author', 'safe', 'on'=>'search'),
		);
	}
