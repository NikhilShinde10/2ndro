...
class DoctorsController extends Controller
{
				...
				if($model->image!="")
					{
						...
						$filen=$model->id."-team";
						$model->image->saveAs("uploads/team/".$filen);	
						$model->image=$filen;
						Yii::app()->db->createCommand("update  doctors set image=\"$filen\" where id=".$model->id)->execute();
					}
				Yii::app()->user->setFlash('success','Doctors Details Added Successfully');	
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Doctors']))
		{
			...
			if($model->save())
			{ 			
				if($model->image!=$newsimg && $model->image!="")
				{	
					$ext = explode(".",$model->image);
					$filen=$model->id."-team";
					$model->image->saveAs("uploads/team/".$filen);	
				...
				Yii::app()->user->setFlash('success','Doctor Details Updated Successfully');	
				$this->redirect(array('view','id'=>$model->id));
			}
		}
